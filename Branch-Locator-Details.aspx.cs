...
public partial class Branch_Loactor_Detailss : System.Web.UI.Page
{
    ...
    {
        string IFSC = Request.QueryString["ifsc"];



        if (SecureQueryString(IFSC) == true)
        {
            filldetails(IFSC);
        }
        else
    ...
    public void filldetails(string id)
    {
        ...
        cmd.Parameters.AddWithValue("@IFSC", id);
        cmd.Parameters["@Query"].Value = "CntBranchIFSC";
        cnt1 = Convert.ToInt32(cmd.ExecuteScalar());
        con.Close();
        if (cnt1 > 0)
        {
            con.Open();
            cmd.Parameters["@Query"].Value = "BranchDetailsIFSC";
            con.Close();
